;(function($, undefined) {
    var kendo = window.kendo,
        ...
        NUMBERINPUTSELECTOR = "[type=number],[type=range],[data-kendo-type=number]",
        BLUR = "blur",
        NAME = "name",
        FORM = "form",
        DATAFOR = "data-kendo-for",
    ...
    var Validator = Widget.extend(/** @lends kendo.ui.Validator.prototype */{ /**
        ...
        options: {
            ...
            rules: {
                ...
                step: function(input) {
                    if (input.filter(NUMBERINPUTSELECTOR).filter("[step]").length && input.val() !== "") {
                        var min = parseInt(input.attr("min"), 10) || 0,
