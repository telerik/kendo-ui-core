    ...
    KendoFile.prototype = {
        ...
        getAMDDeps: cachedProperty("getAMDDeps", function () {
            ...
            try {
                ...
                console.error("*** Can't determine AMD deps for " + this.filename() + ".  Failed to evaluate.");
                console.log("    [", ex, "]");
            }
            return deps;
        }),
...
function U2_parse(code, options) {
    // if (options) {
    //     console.error("--- parsing " + options.filename);
    // }
    try {
        code = code.replace(/\r/g, ""); // <sigh>
...
function listKendoFiles() {
    var js_files = FS.readdirSync(SRCDIR)
        ...
        .filter(function (filename) {
            ...
            if (!has_define) {
                console.error("*** Skipping file " + filename + " (no RequireJS wrapper)");
            }
...
function buildKendoConfig() {
    ...
    files.forEach(function (f) {
        ...
        if (!meta) {
            console.error("*** No __meta__ declaration in " + f);
...
if (require.main === module) (function () {
    ...
    if (ARGV["full"] || ARGV["min"]) {
        var code = loadComponents(files).map(function (f) {
            var comp = getKendoFile(f);
            console.error("Adding " + comp.filename());
            if (comp.isSubfile()) {
                return "";
            } else {
