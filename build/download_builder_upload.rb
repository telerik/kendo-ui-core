...
def upload_download_builder_files()
    ...
    bot.click_element(bot.find_element(:xpath, "//a[contains(, 'Administration')]"))
    click_and_wait("Upload Custom Downloads Package", "Upload Custom Downloads Package")
...
def upload_files_and_test(bot, archive_path)
  ...
  #upload zip file
  full_path = File.expand_path(archive_path + "/#{VERSION}.zip", File.join(File.dirname(__FILE__), ".."))
  ...
  full_path = File.expand_path(archive_path + "/kendo-config.#{VERSION}.js", File.join(File.dirname(__FILE__), ".."))
  ...
  bot.click_element(bot.find_element(:xpath, "//input[contains(@id, '_btnDownload')]"))
