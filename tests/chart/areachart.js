(function() {
    ...
    module("Area Chart / Negative Values", {
        setup: function() {
            setupAreaChart(plotArea, { series: [ negativeSeries ] });
        }
    });
    ...
    test("area has same model id as its segment", function() {
        equal(polyline.style.data.modelId, areaChart._segments[0].options.modelId);
    });
...
(function() {
    ...
    function getElement(modelElement) {
        return $(dataviz.getElement(modelElement.options.id));
    }
