(function() {
    ...
    (function() {
        ...
        test("line has same model id as its segment", function() {
            equal(view.log.path[0].style.data.modelId, lineChart._segments[0].options.modelId);
        });
    ...
    (function() {
        ...
        test("hides marker if size is 0", function() {
            createPoint({ markers: { size: 0 } });
            ok(!marker.options.visible);
        });
        ...
        test("sets marker visibility", function() {
            ...
            equal(marker.options.visible, false);
        });
        ...
        test("marker has same model id", function() {
            ...
            equal(marker.options.modelId, point.options.modelId);
        });
        ...
        test("outline element has same model id", function() {
            ...
            point.highlightOverlay(view);
            equal(view.log.circle[0].style.data.modelId, point.options.modelId);
        });
    ...
    (function() {
        ...
        function getElementFromModel(modelElement) {
            return $(getElement(modelElement.options.id));
        }
        ...
        test("event arguments contain jQuery element", 1, function() {
            linePointClick(function(e) {
                equal(e.element[0], getElement(marker.options.id));
            });
        });
