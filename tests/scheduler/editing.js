(function() {
   var Scheduler = kendo.ui.Scheduler,
    ...
    test("Closing the window does not normalize model if there is no need", function() {
        var scheduler = setup({
                timezone: "Etc/UTC"
            }),
            model = scheduler.dataSource.data()[0],
        ...
        model.set("startTimezone", "Europe/Budapest");
        model.set("endTimezone", "Europe/Budapest");

        scheduler._editor._timezonePopup.find(".k-scheduler-savetimezone").click();

        deepEqual(model.start, new Date(2014, 1, 21, 9));
        deepEqual(model.end, new Date(2014, 1, 21, 9, 30));
    });

    test("Closing the window does not throw exception when update start/end", function() {
        var scheduler = setup({
                timezone: "Etc/UTC"
            }),
            model = scheduler.dataSource.data()[0],
