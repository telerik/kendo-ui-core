...
test("recursive filtering", function() {
    var dataSource = new HierarchicalDataSource({
        data: [
            { text: "foo", items: [
                { text: "bar" },
    ...
    root.children.view()[1].load();

    dataSource.filter({ field: "text", operator: "eq", value: "qux" });

    root = dataSource.view()[0];
    equal(dataSource.view().length, 1);
    equal(root.text, "foo");
    equal(root.children.view().length, 1);

    var baz = root.children.view()[0];
    equal(baz.text, "baz");
    equal(baz.children.view().length, 1);
});

test("recursive filtering, remote data", function() {
    var i = 0;
    var datasource = new HierarchicalDataSource({
        transport: {
            read: function(options) {
                i++;
                options.success([ { hasChildren: true, text: "item " + i } ]);
            }
        },
        schema: { model: { } }
    });
    ...
    var item1 = datasource.view()[0];
    item1.load();

    var item2 = item1.children.view()[0];
    item2.load();

    datasource.filter({ field: "text", operator: "eq", value: "item 3" });

    equal(datasource.view().length, 1);

    item1 = datasource.view()[0];
    equal(item1.text, "item 1");
    equal(item1.children.view().length, 1);

    item2 = item1.children.view()[0];
    equal(item2.text, "item 2");
    equal(item2.children.view().length, 1);
});
