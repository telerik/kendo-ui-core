...
let parseColor = kendo.parseColor;
...
describe("Color", function() {
    ...
    it("Short hex notation", function() {
        let color = parseColor("#ff0");
        assert.isOk(/^#ffff00/i.test(color.toCss()));

        color = parseColor("#0f0");
    ...
    it("Long hex notation", function() {
        ...
        color = parseColor("#223344");
        assert.isOk(/^#223344/i.test(color.toCss()));
    });

    it("RGB notation", function() {
        let color = parseColor("rgb(255, 255, 0)");
        assert.isOk(/^#ffff00/i.test(color.toCss()));

        color = parseColor("rgb(  0,255, 0  )");
        assert.isOk(/^#00ff00/i.test(color.toCss()));

        color = parseColor("rgb( 0000,255,0255\t)");
        assert.isOk(/^#00ffff/i.test(color.toCss()));

        color = parseColor("rgb( 0,\t00,000)");
        assert.isOk(/^#000000/i.test(color.toCss()));

        color = parseColor("rgb(34,51,68)");
        assert.isOk(/^#223344/i.test(color.toCss()));
    });

    it("RGBA notation", function() {
        let color = parseColor("rgba(255, 255, 0, 1)");
        assert.isOk(/^#ffff00/i.test(color.toCss()));

        color = parseColor("rgba(  0,255, 0  , 01  )");
        assert.isOk(/^#00ff00/i.test(color.toCss()));

        color = parseColor("rgba( 0000,255,0255\t, 1\t)");
        assert.isOk(/^#00ffff/i.test(color.toCss()));

        color = parseColor("rgba( 0,\t00,000,1)");
        assert.isOk(/^#000000/i.test(color.toCss()));

        color = parseColor("rgba(34,51,68,1)");
        assert.isOk(/^#223344/i.test(color.toCss()));
    });
});
