...
describe("ProgressBar value", function() {
    ...
    asyncTest("Change event is fired only once per value (type='chunk')", function(done) {
        let changeFiredCounter = 0;
        ...
        setTimeout(function() {
            done(() => assert.equal(changeFiredCounter, 1));
        }, 30);
    });

    asyncTest("Change event is fired only once per value (type='value')", function(done) {
        let changeFiredCounter = 0;
    ...
    asyncTest("Change event is fired only once per value (type='percent')", function(done) {
        let changeFiredCounter = 0;
    ...
    asyncTest("Change event is fired only once per value when trying to set value bigger than max (type='percent')", function(done) {
        let changeFiredCounter = 0;
    ...
    asyncTest("Change event is fired only once per value when trying to set value bigger than max (type='value')", function(done) {
        let changeFiredCounter = 0;
    ...
    asyncTest("Change event is fired only once per value when trying to set value bigger than max (type='chunk')", function(done) {
        let changeFiredCounter = 0;
    ...
    asyncTest("Change event is fired only once per value when trying to set value smaller than min (type='percent')", function(done) {
        let changeFiredCounter = 0;
    ...
    asyncTest("Change event is fired only once per value when trying to set value smaller than min (type='value')", function(done) {
        let changeFiredCounter = 0;
    ...
    asyncTest("Change event is fired only once per value when trying to set value smaller than min (type='chunk')", function(done) {
        let changeFiredCounter = 0;
    ...
    asyncTest("Complete event is not fired before max is reached (type='value')", function(done) {
        ...
        setTimeout(function() {
            done(() => assert.isOk(!completeFired));
        }, 30);
    });

    asyncTest("Complete event is fired when max is reached (type='value')", function(done) {
        let completeFired = false;
    ...
    asyncTest("Complete event is fired each time when max is reached (type='value')", function(done) {
        let completeFiredCounter = 0;
        ...
        setTimeout(function() {
            done(() => assert.equal(completeFiredCounter, 2));
        }, 30);
    });

    asyncTest("Complete event is not fired before max is reached (type='percent')", function(done) {
        let completeFired = false;
    ...
    asyncTest("Complete event is fired when max is reached (type='percent')", function(done) {
        let completeFired = false;
    ...
    asyncTest("Complete event is fired each time when max is reached (type='percent')", function(done) {
        let completeFiredCounter = 0;
    ...
    asyncTest("Complete event is not fired before max is reached (type='chunk')", function(done) {
        let completeFired = false;
    ...
    asyncTest("Complete event is fired when max is reached (type='chunk')", function(done) {
        let completeFired = false;
    ...
    asyncTest("Complete event is fired each time when max is reached (type='chunk')", function(done) {
        let completeFiredCounter = 0;
    ...
    asyncTest("Complete event is not fired max is set multiple times in a row (type='value')", function(done) {
        let completeFiredCounter = 0;
        ...
        setTimeout(function() {
            done(() => assert.equal(completeFiredCounter, 1));
        }, 30);
    });

    asyncTest("Complete event is not fired max is set multiple times in a row (type='percent')", function(done) {
        let completeFiredCounter = 0;
    ...
    asyncTest("Complete event is not fired max is set multiple times in a row (type='chunk')", function(done) {
        let completeFiredCounter = 0;
    ...
    asyncTest("k-progressbar-indeterminate class is set correctly when passed value is false", function(done) {
        pb = new ProgressBar(container, {
            min: 20,
            max: 60,
            value: 20,
        ...
        setTimeout(function() {
            done(() => assert.isOk(pb.wrapper.hasClass("k-progressbar-indeterminate")));
        }, 30);
    });

    asyncTest("k-progressbar-indeterminate class is removed when previous value was false", function(done) {
        pb = new ProgressBar(container, {
            min: 20,
            max: 60,
            value: 20,
    ...
    asyncTest("k-progressbar-indeterminate class is removed when initial value was false", function(done) {
        pb = new ProgressBar(container, {
            min: 20,
            max: 60,
            value: false,
    ...
    asyncTest("One percent is calculated correctly", function(done) {
        pb = new ProgressBar(container, {
            min: 0,
            max: 300,
            value: 0,
        ...
        setTimeout(function() {
            done(() => assert.equal(pb._onePercent, 3));
        }, 30);
    });
});
...
describe("setOptions", function() {
    ...
    it("Private animation object duration is set to 0", function() {
        ...
        let expectedAnimation = { duration: 0 };
