...
let ParentWidget = kendo.ui.Widget.extend({
    options: {
        name: "ParentWidget"
    }
});
...
describe("mvvm binding - no CSP", function() {
    ...
    it("template binding without source does not break observable hierarchy", function() {
        ...
        let model = kendo.observable({ foo: "foo" });
    ...
    it("removing items from data source removes option elements without destroying the existing ones", function() {
        ...
        let firstOption = dom.find("option")[0];
        let lastOption = dom.find("option")[3];
...
describe("mvvm observing - no CSP", function() {
    ...
    it("changing a view model observable array updates the select options within a template", function() {
        ...
        let dom = $('<div data-bind="source: data" data-template="src-bind-template-w-select"></div>');
        kendo.bind(dom, viewModel);
    ...
    it("changing a view model observable array updates only the value bound elements instead of the whole template", function() {
        ...
        let dom = $('<div data-bind="source: data" data-template="src-bind-template"></div>');
        kendo.bind(dom, viewModel);
        viewModel.data[1].value = 1000;
    ...
    it("changing the input value updates dependent observable from the parent object", function() {
        let dom = $('<ul data-bind="source: items" data-template="ul-input-template">');
    ...
    it("tracking changes of observable items in array", function() {
        ...
        let dom = $('<ul data-template="array-template" data-bind="source:foo"/>');
    ...
    it("tracking changes in templates", function() {
        let dom = $('<div data-template="simple-field-template" data-bind="source: this" />');
    ...
    it("dependencies are reavaluated", function() {
        let dom = $('<div data-template="if-else-template" data-bind="source: this" />');

        let viewModel = kendo.observable({ foo: "foo", bar: "bar" });
    ...
    // This test is... horrible. It uses the global scope of the templates to access the templateEvaluationCounter variable, that is why the eslint disablement is needed.
    // If you have time or you're running into problems with it, please see if you can rework it.
    it("does not attach more than one change handler when monitoring for dependency changes", function() {
        let dom = $('<div data-template="counting-template" data-bind="source:this" />');
    ...
    it("refreshing the source destroys the binder objects", function() {
        let dom = $('<div data-template="custom-binder-template" data-bind="source:this" />');
    ...
    it("removing an item from the source destroys the binder objects", function() {
        let dom = $('<div data-template="custom-binder-template" data-bind="source:foo" />');
    ...
    it("tracking changes when direct access and set are used", function() {
        let dom = $('<div data-template="nested-field-template" data-bind="source:this"/>');
    ...
    it("tracking changes when direct access and set are used (multiple get template)", function() {
        let dom = $('<div data-template="nested-field-template-multiple-gets" data-bind="source:this" />');
    ...
    it("tracking changes when single set is used (multiple get template)", function() {
        let dom = $('<div data-bind="source:this" data-template="nested-field-template-multiple-gets" />');
    ...
    it("tracks changes of template bound to parent field", function() {
        let dom = $('<ul data-bind="source: items" data-template="ul-template"></ul>');
    ...
    it("parent properties are invoked with the current item", function() {
        let dom = $('<ul data-bind="source: items" data-template="ul-template-alias"></ul>');
    ...
    it("tracks changes of element bound to parent field within a child template", function() {
        let dom = $('<div><span data-bind="text:root"></span><ul data-bind="source: items" data-template="ul-input-template"></ul></div>');
    ...
    it("tracks changes of element bound to parent nested field within a child template", function() {
        let dom = $('<div><span data-bind="text:root.child"></span><ul data-bind="source: items" data-template="ul-input-child-template"></ul></div>');
