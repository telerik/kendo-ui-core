...
describe("kendo Draggable", function() {
    ...
    asyncTest("the hint is removed from DOM after the draggable is destroyed", function(done) {
        draggable = setup();

        draggable.bind("destroy", function() {
            done(() => assert.equal(hint.parents().length, 0));
        });
    ...
    it("the hint is auto-postioned when dragged", function() {
        ...
        let offset = kendo.getOffset(span);
    ...
    it("the hint can be constrained horizontally", function() {
        ...
        let result = kendo.getOffset(hint);
