...
import { mockFunc, removeMocksIn } from '../../helpers/mock-utils.js';
import { asyncTest } from '../../helpers/async-utils.js';

let menu;
let assertOk = function() { assert.isOk(true); };
...
describe("menu api", function() {
    ...
    it('menu is aligned to the target if alignToAnchor is true', function() {
        ...
        let popupOffset = menu.popup.wrapper.offset(),
    ...
    it('calling open with position shows the popup at that position', function() {
        ...
        let offset = menu.popup.element.offset();
    ...
    it("the element containing the context menu should be the element it was appended to", function() {
        let menuContainer = $('<div id="menu-container">&nbsp;</div>').appendTo(Mocha.fixture);
        let secondContextMenu = $('<div id="context-menu">&nbsp;</div>').appendTo(Mocha.fixture);
        let contextMenu = new kendo.ui.ContextMenu("#context-menu", { target: "#menu-container", appendTo: "#menu-container" });
    ...
    asyncTest("Scrollable context menu renders buttons", function(done) {
        let menuContainer = $(`<div id="target">Target</div>
        ...
        let contextMenu = new kendo.ui.ContextMenu("#context-menu", { target: "#target", scrollable: true });
    ...
    asyncTest("first menu item is focused upon opening", function(done) {
        let target;
        ...
        menu.bind("activate", function() {
            setTimeout(function() {
                done(() => assert.equal(menu.element.find(".k-focus").length, 1));
            });
        });
    ...
    it("ContextMenu is accessible", async function() {
        $("#target").trigger("contextmenu");

        await axeRun(menu.wrapper);
    });

    asyncTest("opened ContextMenu is accessible", function(done) {
        let target;
        ...
        menu.bind("activate", function() {
            setTimeout(async function() {
                await axeRun(menu.wrapper.closest(".k-animation-container").parent());
