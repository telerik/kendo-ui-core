...
let isExpandRaised, isCollapseRaised, isSelectRaised, isActivateRaised;
...
describe("api", function() {
    beforeEach(function() {
        ...
        ul = $('<ul id="panelbar">' +
            ...
            '            </li>' +
            '            <li class="k-panelbar-item"><span class="k-link">Archive <input /></span>' +
            '            </li>' +
            '        </ul>' +
            '    </li>' +
    ...
    asyncTest("expand should raise onActivate event after duration", function(done) {
        ...
        setTimeout(function() {
            done(() => assert.isOk(isActivateRaised));
        }, 400);
    });
    ...
    it('expand method should toggle only one icon', function() {
        let panelbar = new PanelBar(empty_panelbar, { dataSource: [{ text: "Item 1", items: [{ text: "Child 1", items: [{ text: "SubChild" }] }] }] });
    ...
    it('dataSource should create items with the text specified', function() {
        let panelbar = new PanelBar(empty_panelbar, { dataSource: [{ text: "Item 1" }] });

        assert.equal(empty_panelbar.find(".k-panelbar-item > .k-link:contains(Item 1)").length, 1);
    });

    it('dataSource should spawn arrows for items with group, content or contentUrl', function() {
        let panelbar = new PanelBar(empty_panelbar, { dataSource: [{ text: "Item 1", content: "Test" }, { text: "Item 2", items: [{ text: "SubChild" }] }, { text: "Item 3", contentUrl: "http://www.google.com" }] });

        let icons = empty_panelbar.find(".k-panelbar-item > .k-link > .k-icon,.k-panelbar-item > .k-link > .k-svg-icon");
    ...
    it('dataSource should show collapse arrows for expanded items', function() {
        let panelbar = new PanelBar(empty_panelbar, { dataSource: [{ text: "Item 1", content: "Test", expanded: true }] });
    ...
    it("Adding dynamic content element renders properly on root and inner levels", function() {
        ...
        assert.isOk(empty_panelbar.children("li:first").children("div.k-panelbar-content")[0]);
        assert.isOk(empty_panelbar.find("> li:last > ul > li:first").children("div.k-panelbar-content")[0]);
    });
    ...
    it('insertAfter method moves an item if called with existing item', function() {
        let panel = $("<ul><li>Item 1</li><li>Item 2</li></ul>").kendoPanelBar().data("kendoPanelBar");
