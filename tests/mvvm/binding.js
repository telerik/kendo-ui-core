...
test("data attribute binding", function() {
    var dom = $('<a data-bind="attr: { data-foo:foo}"/>');
...
test("binding to array access expression", function() {
    var dom = $('<span data-bind="text: [\'1\']"></span>');
...
test("events binding is detached", 1, function() {
    var dom = $('<span data-bind="events: { click:foo }"/>');
...
test("change binding", 1, function() {
    var dom = $('<input data-bind="events: { change:foo }"/>');
...
test("the context of the nested function call is the nested observable object", 1, function() {
    ...
    var dom = $('<span data-bind="text:nested.foo"/>');
    var viewModel = kendo.observable( {
        nested: {
            foo: function() {
...
test("data source data isn't fetched if the auto-bind attribute is set to false", function() {
    var dom = $('<select data-template="select-template" data-auto-bind="false" data-bind="source:foo"/>');
...
test("value of private field is shown", function() {
    var dom = $('<span data-bind="text:_foo"/>');
...
test("order of source and value does not matter", function() {
    var dom = $('<select data-value-field="name" data-bind="value:selectedItem, source:items"/>');
...
test("binding select value to object specifing only text-field", function() {
    var dom = $('<select data-text-field="name" data-bind="source:items, value:selectedItem"/>');
...
test("binding pre populated select value", function() {
    var dom = $('<select data-bind="value:selectedItem"><option value="foo">foo</option><option value="bar">bar</option>');
...
test("checked binding binds radiobutton to boolean value", function() {
    var dom = $('<input type="radio" value="true" data-bind="checked:selectedItem"/>');
...
test("checked binding binds radiobutton to number value", function() {
    var dom = $('<input type="radio" value="1" data-bind="checked:selectedItem"/>');
...
test("invisible binding hides the element", function() {
    var dom = $('<span data-bind="invisible:hide"/>');
...
test("invisible binding shows the element", function() {
    var dom = $('<span data-bind="invisible:hide" style="display:none" />');
...
test("error is thrown if binding is not find", function() {
    var dom = $('<div data-bind="nonexisting:foo"/>');
...
test("widget initialization", function() {
    ...
    var dom = $('<div data-role="testwidget"></div>');
...
test("child widget is initialized only once", 0, function() {
    ...
    var dom = $('<div data-role="parentwidget"><div data-role="testwidget"></div></div>');
...
test("binding events in template to the root view model", function() {
    ...
    var dom = $('<ul data-bind="source: items" data-template="event-template" />');
...
test("binding events in template to the child view model", function() {
    ...
    var dom = $('<ul data-bind="source: foo.items" data-template="event-template" />');
...
test("binding events in template to the grand child view model", function() {
    ...
    var dom = $('<ul data-bind="source: foo.bar.items" data-template="event-template" />');
...
test("binding template to field of the parent object", function(){
    ...
    var dom = $('<ul data-bind="source: items" data-template="parent-field-template" />');
...
test("binding template to a primitive field", function() {
    ...
    var dom = $('<div data-bind="source: foo.bar" data-template="primitive-template">');
...
test("binding template to a grand parent field", 1, function(){
    ...
    var dom = $('<ul data-bind="source: bar.items" data-template="parent-field-template" />');
...
test("binding template to a grand parent function", function(){
    ...
    var dom = $('<ul data-bind="source: items" data-template="parent-nested-function-template" />');
...
test("widget event binding", 1, function() {
    var dom = $('<div data-role="testwidget" data-bind="events:{ foo: bar }"></div>');
    var viewModel = {
        bar: function() {
...
test("nested template without source with event binding",1, function() {
    var dom = $('<div data-template="nested-master-template" data-bind="source:items"/>');
...
test("text binding displays undefined fields as empty string", function() {
    var dom = $('<span data-bind="text:foo"></span>');
    kendo.bind(dom, {});
...
test("invisible binding to undefined shows the element", function() {
    var dom = $('<span data-bind="invisible:show" style="display:none"/>');
    kendo.bind(dom, {});
...
test("readonly binding to true-ish value sets the readonly attribute", function() {
    var dom = $('<input data-bind="readonly: readonly" />');
...
test("readonly binding to false-ish value removes the readonly attribute", function() {
    var dom = $('<input readonly data-bind="readonly: readonly" />');
...
test("mobile and desktop widgets are initialized in the correct priority", function() {
    ...
    var dom = $("<div data-bind='source: foo' data-template='mobile-widget-template' />");
...
test("binding ignores text nodes", 1, function() {
    var dom = $('<span>Foo</span> <span>Bar</span>');
