...
describe("VirtualList Filtering: ", function() {
            ...
            transport: {
                read: function(options) {
                    setTimeout(function() {
                        ...
                        if (filter) {
                            let filterValue = options.data.filter.filters[0].value;
    ...
    asyncTest("items are rendered after data is filtered", function(done) {
    ...
    asyncTest("itemCount changes after filtering", function(done) {
        asyncDataSource.read().then(function() {
            virtualList.one("listBound", function() {
                done(() => {
    ...
    asyncTest("list renders only the required amount of item placeholders after filtering", function(done) {
        asyncDataSource.read().then(function() {
            virtualList.one("listBound", function() {
                done(() => {
    ...
    asyncTest("works if the dataSource is filtered before list is created", function(done) {
        asyncDataSource.filter({ field: "letter", operator: "eq", value: "a" });
        virtualList.one("listBound", function() {
            done(() => {
                assert.equal(virtualList.items().length, 9);
    ...
    asyncTest("sets the correct container height after filtering", function(done) {
        asyncDataSource.read().then(function() {
            virtualList.one("listBound", function() {
                done(() => {
    ...
    asyncTest("can be scrolled after dataSource is filtered", function(done) {
        asyncDataSource.read().then(function() {
            virtualList.one("listBound", function() {
    ...
    asyncTest("does not clear the values after filtering", function(done) {
        asyncDataSource.read().then(function() {
            virtualList.one("listBound", function() {
                done(() => {
    ...
    asyncTest("highlight selected items when dataSource is filtered", function(done) {
        asyncDataSource.read().then(function() {
            virtualList.one("listBound", function() {
                virtualList.one("listBound", function() {
                    done(() => {
    ...
    asyncTest("select the correct item after filter is cleared", function(done) {
    ...
    asyncTest("throws ListBound event after filter is cleared", function(done) {
        asyncDataSource.read().then(function() {
            virtualList.one("listBound", function() {
                virtualList.one("listBound", function() {
                    done(() => {
...
describe("VirtualList Filtering (local data): ", function() {
    beforeEach(function() {
        ...
        virtualList = new VirtualList(container, {
            ...
            valueMapper: function(options) {
                ...
                let res = [], i, j, l = values.length, dl = data.length;
    ...
    asyncTest("displays result after subsequent filtering", function(done) {
        virtualList.one("listBound", function() {
    ...
    asyncTest("set focus to the first item after subsequent filtering", function(done) {
        localDataSource.filter({ field: "text", operator: "contains", value: "200" });
        ...
        virtualList.one("listBound", function() {
            let that = this;
                ...
                assert.equal(that.focus().first().text(), "Item 1234");
            });
        });
        localDataSource.filter({ field: "text", operator: "contains", value: "1234" });
    });

    asyncTest("selects item that was previously selected after filter is cleared", function(done) {
        virtualList.one("listBound", function() {
    ...
    asyncTest("resets pageSize after filters are cleared", function(done) {
        localDataSource.filter({ field: "text", operator: "contains", value: "200" });
    ...
    asyncTest("clear selected values when list is filtered", function(done) {
        let container = $("<div/>").appendTo(Mocha.fixture);
