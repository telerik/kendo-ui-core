...
describe("Sortable - API", function() {
    ...
    it("start action can be prevented at start event", function() {
        ...
        press(draggedElement, 2, 2);
        move(draggedElement, 100, 100);
        assert.isOk(!sortable.element.find(".item.placeholder").length, "Placeholder must not be initialized");
        assert.isOk(!$(".hint").length, "Hint must not be initialized");
    });
    ...
    it("indexOf method works as expected", function() {
        let item = element.children().eq(1),
