...
import { asyncTest } from '../../helpers/unit/async-utils.js';
    ...
    describe("HierarchicalDataSource", function() {
        ...
        it("Node does not serialize id when idField is changed", function() {
            let NewNode = Node.define({ id: "foo" });

            let node = new NewNode({ foo: 1 });
        ...
        it("the event argument contains the closest node", function() {
            ...
            let child = parent.children.data()[0];
        ...
        asyncTest("error event bubbles from child datasources", function(done) {
            ...
            let dataSource = HierarchicalDataSource.create({
                transport: {
                    read: {
                        url: "bar",
                        dataType: "json"
        ...
        it("getByUid is recursive", function() {
            ...
            let product = dataSource.data()[0].children.data()[0];
        ...
        it("removing and adding the same model does not add multiple change events", function() {
            ...
            let category = dataSource.data()[0];
            dataSource.bind("change", function() {
                counter++;
            });
        ...
        it("infer from list", function() {
            let list = $("<ul><li>foo</li><li>bar</li></ul>"),
        ...
        it("infer from nested list", function() {
            let list = $("<ul><li>foo<ul><li>bar</li></ul></li></ul>"),
                dataSource = HierarchicalDataSource.create({
                    list: list,
                    fields: [{ field: "text" }]
                });
            ...
            let children = data[0].children.data();
        ...
        it("infer from nested list with text wrapper elements", function() {
            let list = $("<ul><li><div>foo</div><ul><li><div>bar</div></li></ul></li></ul>"),
                dataSource = HierarchicalDataSource.create({
                    list: list,
                    fields: [{ field: "text" }]
                });
        ...
        it("infer of hasChildren property", function() {
            let list = $("<ul><li data-hasChildren='true'>foo</li></ul>"),
                dataSource = HierarchicalDataSource.create({
                    list: list,
                    fields: [{ field: "text" }]
                });
        ...
        it("loading items without children does not repopulate their datasource", function() {
            ...
            let children = dataSource.view()[0].children;
            children.read();
        ...
        it("added observable objects can be loaded", function() {
            ...
            let model = kendo.observable({ text: "bar", hasChildren: true });
        ...
        it("get method works with subitems", function() {
            ...
            let children = dataSource.data()[0].children;
            children.read();
    ...
    describe("HierarchicalDataSource : parameterMap", function() {
        ...
        asyncTest("parameterMap gets called when fetching root nodes", function(done) {
            $.mockjax({
                url: "bar", dataType: "json",
                response: function(e) {
                    done(() => assert.equal(e.data.foo, "bar"));
                }
            });
        ...
        asyncTest("parameterMap gets called when fetching child nodes", function(done) {
            $.mockjax({
        ...
        asyncTest("child parameterMap gets called when fetching child nodes", function(done) {
            $.mockjax({
                url: "bar", dataType: "json", data: { id: 1 },
                response: function(e) {
                    done(() => assert.equal(e.data.foo, "bar"));
        ...
        asyncTest("load resolves promise upon success", function(done) {
            let loadPromise;
            ...
            setTimeout(() => {
                loadPromise = dataSource.get(1).load();

                assert.equal(loadPromise.state(), "pending");
                deferred.resolve();

                loadPromise.done(() => {
                    done(() => assert.equal(loadPromise.state(), "resolved"));
                });
            });
        });
