...
describe("kendo.ui.MultiSelect API", function() {
    ...
    asyncTest("MultiSelect value method forces re-bind if autoBind false is used", function(done) {
        let multiselect = new MultiSelect(select, {
            autoBind: false,
            dataValueField: "id",
            dataTextField: "text",
    ...
    asyncTest("MultiSelect value method selects the item retrieved from the server if autoBind:false", function(done) {
        let multiselect = new MultiSelect(select, {
            autoBind: false,
            dataValueField: "id",
            dataTextField: "text",
    ...
    it("MultiSelect supports ObservableArray value", function() {
        popuplateSelect();
        let multiselect = new MultiSelect(select);
        let value = new kendo.data.ObservableArray(["0"]);
    ...
    it("MultiSelect returns array with selected values", function() {
        ...
        let items = multiselect.ul.children();
    ...
    asyncTest("value method selects item that exists only in unfiltered source (async)", function(done) {
        ...
        multiselect.one("dataBound", function() {
            ...
            multiselect.one("dataBound", function() {
                ...
                multiselect.one("dataBound", function() {
                    done(() => assert.deepEqual(multiselect.value(), [2]));
                });
            });
        });
    });
    ...
    it("MultiSelect closes popup", async function() {
        popuplateSelect();
        let multiselect = new MultiSelect(select);
    ...
    it("MultiSelect fetches item if widget is disabled and value is set", function() {
        popuplateSelect();
        let multiselect = new MultiSelect(select.attr("disabled", true), { autoBind: false });
    ...
    it("MultiSelect fetches item if autoBind is set to false when value is set", function() {
        popuplateSelect();
        let multiselect = new MultiSelect(select, { autoBind: false });
    ...
    it("MultiSelect does not allow selection more than maxSelectedItems", function() {
        popuplateSelect();
        let multiselect = new MultiSelect(select, { maxSelectedItems: 1 });
    ...
    it("MultiSelect does not open popup if values >= maxSelectedItems", function() {
        popuplateSelect();
        let multiselect = new MultiSelect(select, { maxSelectedItems: 2 });
    ...
    it("MultiSelect opens popup if noDataTemplate", function() {
        popuplateSelect();
        let multiselect = new MultiSelect(select, { noDataTemplate: () => "no data" });
    ...
    it("enable(false) removes readonly attribute and default class", function() {
        ...
        assert.include(["disabled", ""], multiselect.element.attr("disabled"));
        assert.isOk(multiselect.wrapper.hasClass("k-disabled"));
    });
    ...
    it("enable() allows adding enabled chips", function() {
        popuplateSelect();
        let multiselect = new MultiSelect(select, { enable: false });
        multiselect.enable();
    ...
    it("setOptions updates listView template when dataTextField is set", function() {
        ...
        let result = multiselect.listView.options.template({ anotherName: "abc" });
