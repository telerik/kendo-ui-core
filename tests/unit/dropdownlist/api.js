...
describe("kendo.ui.DropDownList API", function() {
    ...
    it("value method should de-select the selected value if no value", function() {
        ...
        let selectedItems = dropdownlist.ul.children(".k-selected");
    ...
    it("select method passes index -1 when optionLabel is enabled", function() {
        ...
        let listView = dropdownlist.listView;
        ...
        let args = listView.args("select", 0);
    ...
    asyncTest("open method binds the widget and opens the popup", function(done) {
    ...
    it("text method passes object to valueTemplate if no data", function() {
        ...
        dropdownlist.text("Bar");
        let data = dropdownlist.args("valueTemplate")[0];
    ...
    it('after enable(false) should not open popup', function() {
        ...
        let oldOpen = dropdownlist.popup.toggle, called = false;
    ...
    it("readonly() makes  input element readonly", function() {
        ...
        assert.include(["readonly", "true"], dropdownlist.element.attr("readonly"));
    });
    ...
    it("enable(false) removes readonly attribute and default class", function() {
        ...
        assert.equal(dropdownlist.element.attr("readonly"), undefined);
        assert.include(["disabled", "true"], dropdownlist.element.attr("disabled"));
        assert.isOk(dropdownlist.wrapper.hasClass("k-disabled"));
    });
    ...
    it("value method should return value of the SELECT", function() {
        let select = $("<select><option>Chai</option><option selected='selected'>Bar</option></select>").appendTo(Mocha.fixture);
    ...
    it("value method should set value of the SELECT", function() {
        let select = $("<select><option>Chai</option><option>Bar</option></select>").appendTo(Mocha.fixture);
        dropdownlist = new DropDownList(select);
    ...
    it("value method should update only OPTION selected property", function() {
        let select = $("<select><option selected>Chai</option><option>Bar</option></select>").appendTo(Mocha.fixture);
        dropdownlist = new DropDownList(select);
    ...
    it("value() returns value of the OPTION element", function() {
        let select = $("<select><option value=''>Chai</option><option>Bar</option></select>").appendTo(Mocha.fixture);
        dropdownlist = new DropDownList(select);
    ...
    it("dataItem method returns a dataItem based on LI element", function() {
        ...
        let dataItem = dropdownlist.dataItem(dropdownlist.ul.children(":first"));
    ...
    it("dataItem method returns a dataItem based on OptionLabel element", function() {
        ...
        let dataItem = dropdownlist.dataItem(dropdownlist.optionLabel);
    ...
    it("dataItem method returns returns null if argument is null", function() {
        ...
        let dataItem = dropdownlist.dataItem(null);
    ...
    it("DropDownList selects correct item if element is select", function() {
        let select = $('<select><option value="1">United Arab Emirates</option><option selected="selected" value="2">United Kingdom</option><option value="3">United States</option></select>').appendTo(Mocha.fixture);
    ...
    asyncTest("select value after datasource fetch", function(done) {
        ...
        dropdownlist = new DropDownList(input, {
            ...
            dataBound: function() {
                $.mockjax.clear();
                done(() => assert.equal(dropdownlist.value(), "item2"));
            }
        });
    ...
    it("setOptions updates listView template when dataTextField is set", function() {
        ...
        let result = dropdownlist.listView.options.template({ anotherName: "abc" });

        assert.equal(result, "abc");
    });

    it("setDataSource does not trigger cascade event of the parent widget", function() {
        let parent = $("<input id='parent' />").appendTo(Mocha.fixture).kendoDropDownList().data("kendoDropDownList");
    ...
    it("dataItem(element) returns correct dataItem for an item in the first group of a grouped list", function() {
        ...
        // Get the first-group item element using the group-aware listView.items()
        let allItems = dropdownlist.listView.items();
        let alphaEl = allItems.filter(function() { return $(this).text().trim() === "Alpha"; });

        let result = dropdownlist.dataItem(alphaEl);
    ...
    it("dataItem(element) returns correct dataItem for an item in the second group of a grouped list (bug JMC-10448)", function() {
        ...
        // Get a second-group item element using the group-aware listView.items()
        let allItems = dropdownlist.listView.items();
        let gammaEl = allItems.filter(function() { return $(this).text().trim() === "Gamma"; });

        let result = dropdownlist.dataItem(gammaEl);
    ...
    it("dataItem(element) returns correct dataItem for all items across 3 groups (bug JMC-10448)", function() {
        ...
        cases.forEach(function(c) {
            ...
            let result = dropdownlist.dataItem(el);
