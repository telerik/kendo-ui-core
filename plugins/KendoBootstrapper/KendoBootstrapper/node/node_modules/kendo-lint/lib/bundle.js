...
function U2_parse(code, args, errors) {
    try {
        return U2.parse(code, args, errors);
    } catch(ex) {
...
function widget_usage(options) {
    ...
    function ml_handler(args) {
        if (!args) args = {};
        return function(filename){
            var code = FS.readFileSync(filename, { encoding: "utf8" });
            var tree = HTML.parse(code, { asp: args.asp });
            errors.push.apply(errors, tree.errors);
                ...
                if (node.type == "tag") {
                    ...
                    if (node.name == "script") {
                        ...
                        if (!type || /javascript/i.test(type.value)) {
                            do_js_code(node.body.value, filename);
                        }
                        return;
                    }
                }
    ...
    if (options.html_files) {
        options.html_files.forEach(ml_handler());
    }

    if (options.asp_files) {
        options.asp_files.forEach(ml_handler({ asp: true }));
    }
