...
page_title: Axes Configuration | Telerik UI Sparkline HtmlHelper for ASP.NET Core
description: "Learn how to configure the axes in the Telerik UI Sparkline HtmlHelper for ASP.NET Core (MVC 6 or ASP.NET Core MVC)."
...
You can also manually specify the [`BaseUnit`](/api//Kendo.Mvc.UI.Fluent/ChartCategoryAxisBuilder#baseunitkendomvcuichartaxisbaseunit).
...
To access the configuration options, use [`ValueAxis`](/api//Kendo.Mvc.UI.Fluent/SparklineBuilder#valueaxissystemactionkendomvcuifluentchartvalueaxisbuildert). The following example configures a numeric axis with a minimum value of `0` and a maximum value of `100`.
...
* [Basic Usage of the Sparklines HtmlHelper for ASP.NET Core (Demo)](https://demos.telerik.com/aspnet-core/sparklines/index)
