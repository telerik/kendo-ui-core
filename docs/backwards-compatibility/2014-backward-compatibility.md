...
The measure aggregator of the [client pivot cube](/api/data/pivotdatasource/configuration/schema#schemacube) uses **objects** instead of numbers for the `state` argument. The field you should set to accumulate the value is called `accumulator`.
...
* The `activate` event of the TabStrip has been renamed to [`show`](/api/ui/tabstrip/events/show) and fires at the beginning of the open animation. A new [`activate`](/api/ui/tabstrip/events/activate) event has been introduced and is fired at the end of the open animation. This is done for consistency with the rest of the widgets.

* The TabStrip automatically calls [`kendo.resize`](/api/kendo/methods/resize) to its contents in both [`show`](/api/ui/tabstrip/events/show) and [`activate`](/api/ui/tabstrip/events/activate) events.
