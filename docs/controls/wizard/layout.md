...
By default the Wizard is rendered with a [Stepper]({% slug overview_stepper_widget %}) navigation in a horizontal orientation above the content of the Wizard. The layout could be configured via the [`contentPosition`](/api/ui/wizard/configuration/contentposition) configuration property. The available options allow you also to display a vertical Stepper on the left or right side of the Wizard step content.
