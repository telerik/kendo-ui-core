...
You can render the Editor in its read-only state by setting the `contenteditable` attribute of the [`body`](/api/javascript/ui/editor#fields-body) element to `false`.
...
Even though the Editor is read-only, the hyperlinks in its content are active and the user can click them and navigate to the respective page. To disable hyperlinks in the read-only Editor, prevent the clicking of hyperlinks.
    ...
    editorBody.attr("contenteditable", false).find("a").on("click.readonly", false);
