...
By default, the highlight of the ongoing (current) events in the Scheduler is off. In order to enable it you can either set the [ongoingEvents](/api/ui/scheduler/configuration/ongoingEvents) configuration option of the Scheduler to `true`, or set the inner [ongoingEvents.enabled](/api/ui/scheduler/configuration/ongoingevents#ongoingeventsenabled) option to `true`.

## Highlight with No Timezone

The ongoing events highlight functionality will depend on the [timezone](/api/ui/scheduler/configuration/timezone) configured in the Scheduler widget. If no timezone is set on the component, that means the widget uses the local (machine/client) timezone to render its data. As a consequence, whether an event is highlighted or not will depend on its actual duration.
...
If a [timezone](/api/ui/scheduler/configuration/timezone) is specified in the Scheduler widget that will force the ongoing events highlight to take that timezone into account.
