...
For a runnable example, refer to the demo on [binding the Spreadsheet to the DataSource](https://demos.telerik.com/kendo-ui/spreadsheet/datasource). The Spreadsheet DataSource from that example uses the [`read`](/api/data/datasource/configuration/transport#transportread) and [`submit`](/api/data/datasource/configuration/transport#transportsubmit) transport options. The `submit` option is required to properly handle a scenario in which the user creates, updates and deletes items simultaneously.

When using separate [`create`](/api/data/datasource/configuration/transport#transportcreate), [`update`](/api/data/datasource/configuration/transport#transportupdate) and [`destroy`](/api/data/datasource/configuration/transport#transportdestroy) handlers, it is possible that one of them fails, while the others do not. That will result in a mismatch of the data state between the client (the Spreadsheet) and the remote source. The `submit` option handles all operations within a single request. It will not save any changes if any of the items is invalid.
