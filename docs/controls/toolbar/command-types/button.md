...
previous_url: /controls/toolbar/button
...
* `icon`&mdash;The property defines a name of an existing icon from the Kendo UI theme sprite. The icon will be applied as a background image of a `span` element that is rendered inside the Button. For a list of available icon names, refer to the [list of icons](https://www.telerik.com/design-system/docs/foundation/iconography/icon-list).
