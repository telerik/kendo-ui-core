...
You can initially configure the Button as disabled either through its `enable` property or by initializing it from an element which has a `disabled="disabled"` HTML attribute. The Button can also be disabled or enabled at any time with JavaScript by using its `enable()` method with a Boolean argument. For more information on the [`enable` property](/api/ui/button/configuration/enable) and the [`enable` method](/api/ui/button/methods/enable), refer to the [Button API](/api/ui/button).
