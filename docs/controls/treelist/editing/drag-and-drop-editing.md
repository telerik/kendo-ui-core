...
As of Kendo UI R2 SP1 2023, users can reorder the TreeList's rows can be by using the click-move-click functionality provided by the [`clickMoveClick`](/api/ui/treelist/configuration/editable#editablemoveclickmoveclick) option. To start moving the row, users can click the drag icon, and then click again to place the row in its new position.
...
When the [`editable.move`](/api/ui/treelist/configuration/editable#editable.move) option is set to `true`, the user can drag and drop the rows and the TreeList internally updates the `parentId` field. Setting the [`editable.move.reorderable`](/api/ui/treelist/configuration/editable#editable.move.reorderable) property to `true` enables users not only to move items to a different level in the hierarchy but also allows users to reorder items within a particular hierarchy level.
