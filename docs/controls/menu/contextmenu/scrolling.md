...
However, you can enable scrolling by setting the [`scrollable`](/api/ui/contextmenu/configuration/scrollable) property of the ContextMenu to `true`. Once enabled, the ContextMenu displays buttons which scroll the items if they cannot fit the viewport height.
