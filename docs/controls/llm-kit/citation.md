...
The [Citation](/api/ui/citation) component places an inline reference chip directly within AI-generated text. Clicking or hovering the chip opens a popover with complete details for each cited resource, giving readers evidence without leaving the page.

## Configuration

Place the Citation component directly after the claim it supports so the relationship between the statement and its evidence is immediately clear. Provide an array of `CitationSource` objects to the [`sources`](/api/ui/citation/configuration/sources) property to populate the popover with source titles, URLs, and optional descriptions.

Use the [`svgIcon`](/api/ui/citation/configuration/svgicon) and [`sourceIcon`](/api/ui/citation/configuration/sourceicon) properties to customize the chip and popover icons. Set [`showOn`](/api/ui/citation/configuration/showon) to control whether the popover opens on hover or click.
