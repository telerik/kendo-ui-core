...
You can use the [`attributes`](/api/ui/grid/configuration/columns#columnsattributes) function and calculate the span of each cell. In the `attributes` function you can apply the `rowSpan` and `hidden` attributes to the cells.
...
The Column spanning is available through the [`attributes`](/api/ui/grid/configuration/columns#columnsattributes) Grid function where you can apply the `colSpan` and `hidden` attributes to the cells.
