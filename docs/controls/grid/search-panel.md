...
The Grid search functionality is available in the [AI Smart Box](slug:ai_toolbar_tool_kendoui_grid). The Smart Box Search mode uses the same Grid [`search`](/api/ui/grid/configuration/search) configuration, including [`search.fields`](/api/ui/grid/configuration/search#searchfields) and field operators.

To enable it, add the Smart Box tool to the Grid toolbar and configure the [`smartBox.searchSettings`](/api/ui/grid/configuration/smartbox#smartboxsearchsettings) option.
