...
Starting with 2025 Q2 release the Grid component provides the possibility to disable or hide the inactive tools when editing. By default the inactive tools will be hidden. When the [showInactiveTools](/api/ui/grid/configuration/toolbar#toolbarshowinactivetools) option is enabled the inactive tools will be displayed as disabled.
