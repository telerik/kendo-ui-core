...
The following article shows how to make the Kendo UI Splitter expand vertically and resize together with the browser window. It also explains what are the standard requirements in such scenarios.

Web standards require elements with percentage height to have a parent element with an explicit height. The rule applies recursively until an element with a pixel height is reached, or until the `<html>` element is reached. If the requirement is not met, the computed height of the Splitter will fall back to `auto` and the widget will collapse, depending on its content.
...
## Manual resizing to a calculated height
...
###### Example - Splitter with calculated height
