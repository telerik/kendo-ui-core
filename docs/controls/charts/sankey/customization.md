...
You can configure the labels, nodes, and links displayed on the Sankey Diagram by using the following properties:
 ...
 - [`links`](/api/dataviz/ui/sankey/configuration/links)&mdash;Provides options to set the [`colorType`](/api/dataviz/ui/sankey/configuration/links#linkscolortype) of the link to either `static`, `source`, or `target`. It also allows you to control the opacity and highlighting of the link.
 - [`nodes`](/api/dataviz/ui/sankey/configuration/nodes)&mdash;Provides options to modify the `color`, `opacity`, `offset`, `padding`, and `width` of the node.
 - [`labels`](/api/dataviz/ui/sankey/configuration/labels)&mdash;Provides options to modify the `font`, `color`, and `opacity` of the label. It also allows you to set the `visible` property to show or hide the label.

## Configure the Link Colors

The Sankey diagram provides different [`colorTypes`](/api/dataviz/ui/sankey/configuration/links#linkscolortype) of the links. The supported values are:
...
The example below demonstrates the different [`colorTypes`](/api/dataviz/ui/sankey/configuration/links#linkscolortype). You can select a `colorType` from the DropDownList at the top:
...
As the properties set to [`data`](/api/dataviz/ui/sankey/configuration/data) take precedence, using the `data` property allows you to target a single node or link and apply a specific configuration only to that element.
