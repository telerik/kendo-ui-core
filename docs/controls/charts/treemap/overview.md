...
To set different colors to sibling `leaf` items without having the colors in the data, use the [`itemCreated`](/api/dataviz/ui/treemap/events/itemcreated) or the [`dataBound`](/api/dataviz/ui/treemap/events/databound) event of the TreeMap.
...
You can refer an existing TreeMap instance by using [`jQuery.data()`](https://api.jquery.com/jQuery.data/). Once a reference is established, use the [TreeMap API](/api/dataviz/ui/treemap) to control its behavior.
...
* [JavaScript API Reference of the TreeMap](/api/dataviz/ui/treemap)
