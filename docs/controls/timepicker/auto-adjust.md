...
Starting with Kendo UI R3 2024, the TimePicker component provides an [`autoAdjust`](/api/ui/timepicker/configuration/autoadjust) feature which provides control over the autocorrect functionality. By default, when you have the `min` and `max` time slots configured and the [`dateInput`](/api/ui/timepicker/configuration/dateinput) is set to `true`, the component will autocorrect the user input if it is not within the specified range. The autocorrected value will be either the `min` or the `max` value you have set for the component depending on which the invalid value is closer to.
