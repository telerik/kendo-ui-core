---
title: PDF Output
page_title: PDF Output | Kendo UI Drawing Library
...
The [Kendo UI Drawing library](http://demos.telerik.com/kendo-ui/drawing/index) can export your drawing to a PDF file.

## Employing Functions

Since PDF files cannot be displayed by a browser inside an HTML element, you cannot create a `Surface` object for this kind of output. Use a few functions that are exported to `kendo.drawing.pdf` to generate the binary data instead.
...
The `proxyUrl` and `callback` arguments are optional. The `proxyUrl` is necessary for the download to work with Internet Explorer 9 and Safari and is not going to be used for other browsers. The `callback` will be invoked when the file has been successfully generated (generation could be asynchronous). For more information on the `proxyURL`, refer to [`kendo.saveAs`](/api/javascript/kendo/methods/saveas).
...
The Kendo UI Drawing API allows you to specify fonts with the `font` option of `Text` elements.
...
The following example demonstrates how to manually define fonts.
...
Run this code before a PDF is requested. You can include it into a `<script>` tag in your page. The object that is passed to `kendo.pdf.defineFont` must map between font name/style to an URL with the `TrueType` file. The same origin policy applies, so you cannot specify URLs to different hosts.
...
Currently, only TTF fonts having an Unicode mapping are supported. If you do not declare any fonts, the Kendo UI PDF generator will fallback to the following standard PDF fonts.
