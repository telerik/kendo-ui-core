...
The [`kendo.format`](/api/kendo/methods/format) and [`kendo.toString`](/api/kendo/methods/tostring) methods support standard and custom date formats.

The following runnable example demonstrates how to convert a JavaScript `Date` object to a string by using the [`kendo.toString`](/api/kendo/methods/tostring) method:
...
| Specifier     | Result  |
|:---           |:---     |
|`standard`     |Retrieved from the used Kendo UI culture script that is defined with the [`kendo.culture`](/api/kendo/methods/culture) method.
