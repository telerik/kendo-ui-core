...
The demo implies the usage of incell editing, built-in keyboard navigation, and the [`autoSync:true`](/api/data/datasource/configuration/autosync) configuration for the Grid [DataSource instance](/api/data/datasource).
...
Generally, it is uncommon to enable the [`autoSync`](/api/data/datasource/configuration/autosync) option for the incell editing because it greatly increases the amount of update-related remote requests. Nevertheless, it is possible for you to use such an approach if required.
