...
1. To achieve the desired behavior you can add a [`footerTemplates`](/api/ui/grid/configuration/columns#columnsfootertemplate) in the columns with an empty `span` element with custom class.
1. You can also add custom commands to the [`contextMenu.body`](/api/ui/grid/configuration/contextmenu#contextmenubody).
