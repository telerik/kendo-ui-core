...
The template is not automatically rebound after drag-and-drop and this issue is expected. To work around it, manually refresh the second ListBoxt on the [`dragend`](https://docs.telerik.com/kendo-ui/api/ui/listbox/events/dragend) event of the first ListBox.

For the full implementation, refer to [this Dojo example](https://dojo.telerik.com/uPowuQoq). If the arrows from the toolbar are used, you need to apply similar logic to the [`add`](https://docs.telerik.com/kendo-ui/api/ui/listbox/events/add) event (with timeout).
