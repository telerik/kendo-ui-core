...
1. Subscribe to the [`remove`](/api/ui/gantt/events/remove) event and prevent the default behavior.
...
1. The Kendo UI Gantt's [`editable.confirmation`](/api/ui/gantt/configuration/editable#editableconfirmation) should be set to `false` to prevent the default confirm window from showing.
