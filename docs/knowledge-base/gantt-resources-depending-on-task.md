...
1. Handle the [`edit`](https://docs.telerik.com/kendo-ui/api/ui/gantt/events/edit) event of the Gantt.
1. In the `edit` event, retrieve and store the start and end date in global variables.
1. Trigger the [`read()`](https://docs.telerik.com/kendo-ui/api/data/datasource/methods/read) call on the Resources DataSource.
...
1. Configure a [`Data()`](https://docs.telerik.com/kendo-ui/api/data/datasource/configuration/transport#transportread#transportreaddata) function for the Resources Read action. As a result, you will be able to filter the returned Resources based on the start and end time of the task on the server.
