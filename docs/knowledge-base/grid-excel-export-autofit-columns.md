...
Set the column width of the Kendo UI Workbook to [`autoWidth`](/api/ooxml/workbook/configuration/sheets.columns#sheetscolumnsautowidth) by adding a handler to the [`excelexport`](https://docs.telerik.com/kendo-ui/api/ui/grid/events/excelexport) event of the Grid. Loop over the columns and set `autoWidth` to `true`.
