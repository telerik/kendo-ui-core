...
The `FileEntry` is an extended type of a [`Node`](/api/framework/node) that works with hierarchical data. The [FileManagerDataSource](/api/data/filemanagerdatasource) contains instances of the `FileEntry` type.
...
The child `kendo.data.FileManagerDataSource` of the node. This field is initialized lazily if the `hasChildren` or `isDirectory` field is set or when the [`load`](/api/data/node/methods/load) or [`append`](/api/data/node/methods/append) methods were called.
