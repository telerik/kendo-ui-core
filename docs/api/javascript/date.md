...
##### direction `Number`
The direction that determines the day of the week to be set. 1 indicates the day after the current one (default value); -1 indicates the day before the current one.
...
### dayOfWeek
Return a new date object that is set with a particular day of the week.
...
### getDate
Gets the date only (i.e., removes the time part).
...
#### Returns
`Boolean` Return true if the target date is within the date range.
...
### isInTimeRange
Determines whether the time part of a date is within the time span of a certain date range.
...
#### Returns
`Boolean` Return true if the time part of a date is within the time span of a certain date range.
...
##### ignoreDST `Boolean`
A boolean value that indicates whether the DST will be ignored. Default value is false.
...
#### Returns
`Date` Returns a new date object with replaced time part from another date object.
...
### toInvariantTime
Returns the invariant time (i.e., the time added to 1980th year) of a date object.
...
#### Returns
`Date` Returns the invariant time (i.e., the time added to 1980th year) of a date object.
