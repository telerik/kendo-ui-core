...
Segments are created implicitly by the [Path](https://www.telerik.com/kendo-jquery-ui/documentation/api/drawing/path) lineTo and curveTo commands.
...
How do I set the anchor point for a path segment in Kendo UI drawingapi? Set or configure the anchor point for a path segment to control where the segment passes through or ends within a drawing or vector graphic component, enabling alignment, connection, or positioning of segment endpoints by specifying the key coordinate used as the reference point; this can be used to define segment placement precisely, move endpoints to desired locations, link segments smoothly, or adjust path shapes by setting or adjusting the anchor parameter or coordinate value that the segment path follows or terminates at.
</div>
