...
Triggered when the user presses a keyboard key while the TileLayout is focused.

The event handler function context (available via the `this` keyword) will be set to the widget instance.

<div class="meta-api-description">
How do I handle keyboard events in Kendo UI TileLayout? Capture and intercept keydown events fired while the TileLayout is focused, enabling custom keyboard navigation, overriding default key behaviors, preventing built-in keydown logic with the preventKendoKeydown flag, canceling native browser actions via preventDefault, and implementing custom keyboard shortcuts or accessibility enhancements within the TileLayout component.
</div>
...
If set to `true` prevents the default TileLayout keydown logic.
...
#### Example - subscribe to the "kendoKeydown" event during initialization
    ...
    <script>
      $("#tilelayout").kendoTileLayout({
        containers: [
          { colSpan: 1, rowSpan: 1, header: { text: "Container 1" }, bodyTemplate: "Content 1" },
          { colSpan: 1, rowSpan: 1, header: { text: "Container 2" }, bodyTemplate: "Content 2" }
        ],
...
#### Example - subscribe to the "kendoKeydown" event after initialization
    ...
    <script>
      ...
      var widget = $("#tilelayout").data("kendoTileLayout");
