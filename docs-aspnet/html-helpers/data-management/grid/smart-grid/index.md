---
title: Overview
description: "Discover how the {{ site.framework }} Smart Grid combines AI-powered features with traditional Grid functionality to enable natural language interaction and intelligent data exploration."
slug: overview_smart_grid
position: 0
components: ["grid"]
---

# {{ site.framework }} Smart Grid Overview

The Smart Grid enhances the traditional [{{ site.framework }} Grid]({% slug htmlhelpers_grid_aspnetcore_overview %}) with AI-powered capabilities that enable users to interact with data using natural language. These capabilities transform how users explore and manipulate Grid data, making complex operations more intuitive and accessible.
...
<Row>
  ...
  <Column count={[24,24,12]}>
    <Component className="tile pd-b" href="https://demos.telerik.com/aspnet-core/grid/ai-toolbar-highlight">
  ...
  <Column count={[24,24,12]}>
    <Component className="tile pd-b" href="slug:custom_ai_column_core_grid">
...
The [AI Smart Box](slug:ai_toolbar_tool_core_grid) is a versatile search box that combines traditional search functionality with AI-powered capabilities. It provides three distinct modes that you can enable independently or in combination:

- [Search Mode](slug:ai_toolbar_tool_core_grid)&mdash;Traditional keyword-based filtering across the Grid columns.
- [Semantic Search Mode](slug:ai_toolbar_tool_core_grid)&mdash;Enhanced search that interprets user intent and matches related terms, synonyms, and contextual meanings to find semantically relevant data in the Grid.
- [AI Assistant Mode](slug:ai_toolbar_tool_core_grid)&mdash;Full AI-powered Grid control with the same capabilities as the [AI Toolbar Assistant](slug:ai_toolbar_tool_core_grid). Users can perform any [supported Grid operation](#supported-operations) through natural language prompts entered in the AI Assistant mode of the Smart Box.
...
>tip For an enhanced user experience with additional search capabilities, prompt suggestions, and streamlined UI, we recommend using the [AI Smart Box](slug:ai_toolbar_tool_core_grid). The AI Smart Box combines traditional search, semantic search, and AI-powered operations in a single, unified interface.

The [AI Toolbar Assistant](slug:ai_toolbar_tool_core_grid) is a dedicated toolbar button with a distinctive AI sparkles icon that provides a streamlined interface for AI-powered Grid control, allowing users to enter natural language prompts to perform Grid operations.
...
> For detailed implementation instructions, including package installation, AI provider configuration, and controller setup, see [AI Service Setup](slug:smart_ext_core_grid).
...
Use the detailed information about the expected request and response structures from the [AI Service Setup](slug:smart_ext_core_grid#request-and-response-format) article to build a compatible custom service.
...
* [AI Column Assistant](slug:custom_ai_column_core_grid)
