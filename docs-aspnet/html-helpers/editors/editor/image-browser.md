...
To retrieve and upload the files and directories, the image browser needs a server-side implementation. To configure the image browser tool, use the [`ImageBrowser()`](/api/kendo.mvc.ui.fluent/editorbuilder#imagebrowsersystemaction) method. 
...
You can update all of these requests and responses through the [`ImageBrowser()`](/api/kendo.mvc.ui.fluent/editorbuilder#imagebrowsersystemaction) configuration. Each of them exposes more options that you can tweak.
