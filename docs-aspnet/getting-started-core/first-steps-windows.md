...
meta_title: First Steps with Telerik UI for ASP.NET Core in VS for Windows | Telerik UI for ASP.NET Core
...
1. Register the Kendo UI service in the services container.
	...
	* For applications using .NET 6 or later and the [minimal hosting model](https://learn.microsoft.com/en-us/aspnet/core/migration/50-to-60?view=aspnetcore-10.0&viewFallbackFrom=visual-studio&tabs=visual-studio#new-hosting-model), open the `Program.cs` file and register the Kendo UI service.
...
1. The ASP.NET Core Web App template comes with a jQuery script reference at the end of `_Layout.cshtml` file. Locate the `jquery.min.js` script line in the `<body>` of the document and move it to the `<head>`. Alternatively, use the jQuery script hosted on the jQuery CDN.
...
* [MS - Upgrade from ASP.NET Framework to ASP.NET Core](https://learn.microsoft.com/en-us/aspnet/core/migration/fx-to-core/?view=aspnetcore-10.0&viewFallbackFrom=aspnetcore-7.0)
