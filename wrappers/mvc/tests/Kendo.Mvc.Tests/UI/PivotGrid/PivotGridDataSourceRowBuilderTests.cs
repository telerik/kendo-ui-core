namespace Kendo.Mvc.UI.Fluent.Tests
{
    using Xunit;

    public class PivotGridDataSourceRowBuilderTests
    {
        private readonly PivotGridDataSourceRow row;
        private readonly PivotGridDataSourceRowBuilder builder;

        public PivotGridDataSourceRowBuilderTests()
        {
            row = new PivotGridDataSourceRow();
            builder = new PivotGridDataSourceRowBuilder(row);
        }

        [Fact]
        public void Row_expand_sets_the_corresponding_property()
        {
            bool isExpanded = true;
            builder.Expand(isExpanded);

            Assert.Equal(isExpanded, row.Expand);
        }

        [Fact]
        public void Row_expand_should_return_builder()
        {
            builder.Expand(true).ShouldBeSameAs(builder);
        }
    }
}
